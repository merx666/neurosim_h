...
// ⚡ Bolt: Move expensive static data grouping outside component
// Prevents O(N) recalculation of SUBSTANCES on every render
const groupedSubstances = Object.values(SUBSTANCES).reduce((acc, s) => {
...
export const SubstanceGrid: React.FC<SubstanceGridProps> = ({ onSelect }) => {
  ...
  return (
    <div className="substance-grid-container animate-fade-in">
      {Object.entries(CATS).map(([catId, catData]) => {
        const categorySubstances = groupedSubstances[catId];
        if (!categorySubstances) return null;
        
        const catName = language === 'pl' ? (catData as any).label : (catData as any).label_en;

...
export function MixExplorer() {
  ...
  // ⚡ Bolt Optimization:
  // Memoize filtered substances to avoid O(N) recalculations on every render.
  // Hoist `toLowerCase()` outside the `.filter` loop to prevent redundant string
  // allocations, reducing GC pressure and CPU cycles when filtering large arrays.
  const filteredA = useMemo(() => {
    const term = searchA.toLowerCase();
    return substances.filter(s =>
      s.name_pl.toLowerCase().includes(term) ||
